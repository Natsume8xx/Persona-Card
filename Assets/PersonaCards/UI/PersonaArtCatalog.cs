...
namespace PersonaCards.UI
{
    ...
    /// 序号与网页版冻结导出 basePersonas（internalIds 数组序）一致：
    ///   01 observer / 02 wanderer / 03 pathfinder / 04 restraint
    ///   05 collector / 06 resonance / 07 commitment / 08 purger
    /// 中文名以配表 图片配置 表 IMAGE_064~071（人格牌01~08 原画）＝ PER_001~PER_008 为准；
    ...
    public static class PersonaArtCatalog
    {
        ...
        private static readonly Dictionary<string, string> KeyToArtNumber = new Dictionary<string, string>
        {
            ...
            // 映照→06 号美术、偏转→03 号美术、裂变→暂无裂变美术，暂用 01 号美术
